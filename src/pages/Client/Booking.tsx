...
export function ClientBooking() {
  ...
  // Effect to generate dates only AFTER business hours are loaded
  useEffect(() => {
    if (Object.keys(businessHours).length > 0) {
      generateDates(businessHours);
    }
  }, [businessHours]);
  ...
  function generateDates(hoursInfo: BusinessHours) {
    const arr = [];
    let d = new Date();
    // Gera próximos 14 dias válidos baseados no businessHours
    // Para evitar loops infinitos se todos os dias estiverem fechados, limitaremos a busca
    let attempts = 0;
    while (arr.length < 14 && attempts < 60) {
      const day = d.getDay();
      const config = hoursInfo[day];
  ...
  async function generateSlots(date: Date, durationStr: number) {
    try {
      ...
      // Parse business hours openTime and closeTime
      ...
      const nowAjusted =
        offset === 0 ? new Date(now.getTime() - 3 * 60 * 60 * 1000) : now;
      ...
      const validSlots = slots.filter((slot) => {
        ...
        // Final slot cannot exceed closing time by more than 1 minute
        if (slotEnd > new Date(end.getTime() + 1 * 60000)) return false;

        // Break logic
        const fallsInBreak = dayConfig.breaks.some((brk) => {
          const [brkStartH, brkStartM] = brk.start.split(":").map(Number);
          const [brkEndH, brkEndM] = brk.end.split(":").map(Number);

          const breakStartOnDay = setMinutes(
            setHours(baseDate, brkStartH),
            brkStartM,
          );
          const breakEndOnDay = setMinutes(
            setHours(baseDate, brkEndH),
            brkEndM,
          );

          // If slot overlaps with break: (StartA < EndB) and (EndA > StartB)
          return slot < breakEndOnDay && slotEnd > breakStartOnDay;
        });

        if (fallsInBreak) return false;
  ...
  async function handleBook() {
    ...
    const endTime = new Date(
      selectedSlot.getTime() + selectedService.duration * 60000,
    );
    ...
    try {
      ...
      if (hasConflict) {
        alert(
          "Infelizmente este horário acabou de ser reservado. Por favor, escolha outro.",
        );
        generateSlots(selectedDate!, selectedService.duration);
        setIsBooking(false);
        return;
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-white/60 max-w-md">
            Sua solicitação de horário para{" "}
            {selectedSlot instanceof Date
              ? format(selectedSlot, "dd/MM 'às' HH:mm", { locale: ptBR })
              : "--:--"}{" "}
            foi enviada ao barbeiro. Aguarde a aprovação na aba Meus Cortes.
          </p>
        </div>
        ...
        <div className="w-full max-w-md bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-lg flex flex-col items-center mt-6">
          ...
          <p className="text-sm text-white/60 mb-4 text-center">
            Para agilizar, você já pode efetuar o pagamento do valor de R${" "}
          ...
          <div className="text-center space-y-4">
            <p className="text-xs text-white/40">
              ⚠️ Importante: Após o pagamento, você{" "}
              <strong>deve enviar o comprovante</strong> pelo WhatsApp para
    ...
    <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="bg-amber-500/20 text-amber-500 p-4 border-b border-amber-500/20">
            ...
            <p className="text-sm mt-1 opacity-80">
              {confirmedAppointment.services?.name} em{" "}
          ...
          <div className="p-5 space-y-4">
            <div className="flex flex-col items-center gap-2">
              <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">
                Pague para confirmar definitivamente
              </span>
      ...
      <div>
        ...
        <p className="text-white/40 text-sm">
          Siga os 3 passos para reservar sua cadeira.
        </p>
      </div>
      ...
      <div className="space-y-3">
        <h2 className="text-lg font-semibold flex items-center gap-2 text-white">
          <div className="w-6 h-6 rounded-full bg-amber-500/20 text-amber-500 flex items-center justify-center text-xs font-bold border border-amber-500/50">
            1
          </div>
            ...
            {services.map((srv) => (
              ...
              >
                ...
                <div className="font-bold text-amber-500">
                  R$ {srv.price ? Number(srv.price).toFixed(2) : "0.00"}
                </div>
              </button>
            ))}
          </div>
      ...
      {selectedService && selectedDate && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-white/40">
              Horários aguardando aprovação não são exibidos.
            </p>
          </div>
